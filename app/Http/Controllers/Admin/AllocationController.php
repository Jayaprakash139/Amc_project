...
class AllocationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $assets = Allocation::with(['school', 'aaset_category','status','custody','sublocation','amcStatus'])->get();


        return view('admin.allocations.index',compact('assets'));
    }
    ...
    public function create()
    {
        ...
        $sub_locations = Location::pluck('sub_location', 'id')->prepend(trans('global.pleaseSelect'), '');
        ...
        return view('admin.allocations.create', compact('aaset_categories', 'custodies', 'schools', 'statuses', 'locations','sub_locations'));
    }
    ...
    public function edit(Allocation $allocation)
    {
        ...
        // Make sure you are loading the customFields relationship correctly
        $allocation->load('school', 'sublocation', 'aaset_category', 'status', 'custody', 'team', 'customFields');
        //dd($allocation);
        return view('admin.allocations.edit', compact('aaset_categories', 'allocation', 'custodies', 'schools', 'statuses', 'locations','sub_locations'));
    }
    ...
    public function update(UpdateAllocationRequest $request, Allocation $allocation)
    {
        ...
        // foreach ($req->product_name as $count => $productName) {
        //     $productDetail = [
        //         'sale_id' => $req->sale_id,
        //         'category_id' => $req->category_name[$count],
                
        //     ];
        
        //     // Use the firstOrNew method to either create a new row or update an existing one
            $existingProductDetail = Customfield::firstOrNew(['allot_id' => $request->unique_id, 'name' => $request->name[$count]]);
            $existingProductDetail->fill($dataToSave);
    ...
    public function updateAmcStatus(Request $request, Allocation $allocation)
{
    abort_if(Gate::denies('allocation_edit'), 403);

    $request->validate([
        'amc_status_id' => 'required|exists:amc_statuses,id',
    ]);

    $allocation->update([
        'amc_status_id' => $request->amc_status_id,
        'amc_updated_at' => now(),
    ]);

    return redirect()->back()->with('message', 'AMC Status updated successfully.');
}
...
public function getSublocations($locationId)
{
    ...
    // If the location exists and has a sub_location value
    if ($location && $location->sub_location) {
        return response()->json([
            $location->id => $location->sub_location
        ]);
    }
