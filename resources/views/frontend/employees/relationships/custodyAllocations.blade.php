    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-custodyAllocations">
                    <thead>
                        <tr>
                            <th width="10">
                    ...
                    <tbody>
                        ...
                        @foreach($allocations as $key => $allocation)
                            <tr data-entry-id="{{ $allocation->id }}">
                                ...
                                <td>
                                    {!! QrCode::size(70)->generate('AMS UId: ' . $allocation->unique_id . ', Asset Name: '.$allocation->asset_name . ', Asset Code: '.$allocation->asset_code . ', School: ' .  $allocation->school->name . ', Sublocation: '.$allocation->sublocation->name . ', Value: '.$allocation->value . ', Status: '.$allocation->status->name . ', Custody: '.$allocation->custody->name  )!!}  
                                ...
                                <td>
                                    @can('allocation_show')
                                        <a class="btn btn-xs btn-primary" href="{{ route('frontend.allocations.show', $allocation->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan

                                    @can('allocation_edit')
                                        <a class="btn btn-xs btn-info" href="{{ route('frontend.allocations.edit', $allocation->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
...
@endcan
  ...
  let table = $('.datatable-custodyAllocations:not(.ajaxTable)').DataTable({ buttons: dtButtons })
