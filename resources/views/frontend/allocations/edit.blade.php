...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.allocations.update", [$allocation->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="sublocation_id">{{ trans('cruds.allocation.fields.sublocation') }}</label>
                <select class="form-control select2 {{ $errors->has('sublocation') ? 'is-invalid' : '' }}" name="sublocation_id" id="sublocation_id" required>
                    @foreach($sublocations as $id => $entry)
                        <option value="{{ $id }}" {{ (old('sublocation_id') ? old('sublocation_id') : $allocation->sublocation->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="value">{{ trans('cruds.allocation.fields.value') }}</label>
                <input class="form-control {{ $errors->has('value') ? 'is-invalid' : '' }}" type="number" name="value" id="value" value="{{ old('value', $allocation->value) }}" step="1">
                @if($errors->has('value'))
            ...
            <div class="form-group">
                <label class="required" for="status_id">{{ trans('cruds.allocation.fields.status') }}</label>
